...
DR_drifting_base <- function( pinputexps, metodo)
{
  ...
  param_local$meta$script <- "/src/wf-etapas/todas_las_variables_1401_DR_corregir_drifting.r"
...
wf_septiembre <- function( pnombrewf )
{
  ...
  fm <- FM_final_models_lightgbm( c(ht, ts9), ranks=c(1), qsemillas=10 ) #sumar más semillas
  SC_scoring( c(fm, ts9) )
  KA_evaluate_kaggle()
