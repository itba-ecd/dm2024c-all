...
Corregir_spline <- function(pcampo, pmeses) {
  
  # Crea x-valores
  x_values <- seq_len(nrow(dataset))
  
  # Crea y-valores para el campo seleccionado donde se identificaron variables rotas
  y_values <- dataset[[pcampo]]
  
  # Group by entity ID (client) and apply spline fitting for each group
  tbl <- dataset[, list(
    
    # Identify valid (non-zero) values for the variable
    valid_values = get(pcampo) != 0,
    
    # Fit the spline using valid data points (non-zero) with 3 degrees of freedom
    spline_fit = {
      if (sum(get(pcampo) != 0) > 3) {  # Ensure there are enough valid points for fitting
        # Fit the spline using valid y-values and their x-positions
        spline_model <- smooth.spline(x_values[get(pcampo) != 0], get(pcampo)[get(pcampo) != 0], df = 3)
        # Predict the values for all rows (interpolate where necessary)
        predict(spline_model, x_values)$y
      } else {
        # If there aren't enough valid values, keep the original values
        get(pcampo)
      }
    }
  ), by = eval(envg$PARAM$dataset_metadata$entity_id)]  # Group by entity ID (client)
  
  # Remove the entity ID column from the temporary table to avoid duplication
  tbl[, paste0(envg$PARAM$dataset_metadata$entity_id) := NULL]
  
  # Update the dataset for the specified months (pmeses)
  dataset[
    ,
    paste0(pcampo) := ifelse(foto_mes %in% pmeses,
...
Corregir_atributo <- function(pcampo, pmeses, pmetodo)
{
  ...
  switch( pmetodo,
    ...
    "Spline"              = Corregir_spline(pcampo, pmeses)  # Anado el metodo Spline
  )
