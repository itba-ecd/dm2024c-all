...
PARAM$semilla_primigenia <- 176573
PARAM$qsemillas <- 20
...
for (vmax_depth in c(4, 6, 8, 10, 12, 14)) {
  ...
  for (vmin_split in c(1000, 800, 600, 400, 200, 100, 50, 20, 10)) {
    print("vmin_split")
    print(vmin_split)
    for (cp_value in c(-1, -0.5, 0)) {
      print("cp_value")
      print(cp_value)
      for (minbucket_value in c(2, 3, 5)) {
        ...
        param_basicos <- list(
          "cp" = cp_value, # complejidad minima
          "maxdepth" = vmax_depth, # profundidad máxima del arbol
          "minsplit" = vmin_split, # tamaño minimo de nodo para hacer split
          "minbucket" = minbucket_value # minima cantidad de registros en una hoja
        )
