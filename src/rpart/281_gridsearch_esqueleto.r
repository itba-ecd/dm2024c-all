...
# Definimos los valores para cada hiperparámetro
cp_values <- c(0.01, 0.1)         # Menor cantidad de valores para cp
maxdepth_values <- c(4, 6)        # Menor profundidad máxima
minsplit_values <- c(200, 100)    # Menor cantidad de registros para hacer split
minbucket_values <- c(5, 10)      # Menor cantidad de registros en una hoja

# Iteramos sobre todos los hiperparámetros
for (vcp in cp_values) {
  for (vmax_depth in maxdepth_values) {
    for (vmin_split in minsplit_values) {
      for (vmin_bucket in minbucket_values) {
        ...
        # Agregamos a la tabla de resultados
  ...
  # Grabamos cada vez toda la tabla en el loop más externo
  fwrite(tb_grid_search_detalle, file = "gridsearch_detalle.txt", sep = "\t")
}

#----------------------------

# Generamos y grabamos el resumen
...
# Ordenamos descendente por ganancia
