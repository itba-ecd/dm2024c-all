...
# Aquí se cargan los bordes de los hiperparámetros
hs <- makeParamSet(
  makeNumericParam("learning_rate", lower = 0.01, upper = 0.3),         # Tasa de aprendizaje
  makeIntegerParam("num_leaves", lower = 8L, upper = 1024L),            # Número de hojas
  makeNumericParam("feature_fraction", lower = 0.1, upper = 1.0),       # Fracción de características a usar
  makeIntegerParam("min_data_in_leaf", lower = 1L, upper = 8000L),      # Mínimo de datos en las hojas
  ...
  makeNumericParam("bagging_fraction", lower = 0.5, upper = 1.0),       # Fracción de datos para bagging
  makeIntegerParam("bagging_freq", lower = 1L, upper = 10L),            # Frecuencia de bagging
  
  # Nuevos hiperparámetros
  makeIntegerParam("min_child_samples", lower = 1L, upper = 100L),      # Mínimo de muestras en un nodo hijo
  makeIntegerParam("max_bin", lower = 10L, upper = 255L),               # Máximo número de bins para discretización
  makeIntegerParam("num_iterations", lower = 100L, upper = 10000L),     # Número de iteraciones
  makeIntegerParam("n_estimators", lower = 5000L, upper = 15000L),      # Sinónimo de "envios", manteniéndolo para consistencia
  makeIntegerParam("max_bin_1", lower = 5L, upper = 256L),              # Otra opción para discretización de características
  makeNumericParam("subsample", lower = 0.1, upper = 1.0),              # Submuestreo de datos
  makeNumericParam("colsample_bytree", lower = 0.1, upper = 1.0)        # Submuestreo de columnas por árbol
)
